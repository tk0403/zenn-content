...
この記事は、[TiDB Advent Calendar 2023](https://qiita.com/advent-calendar/2023/tidb) 17日目の記事です。

# はじめに
TiDBはMySQLと互換性を持ったNewSQLデータベースということで、MySQLユーザーで気になっている方はいらっしゃるんじゃないかと思います。
...
まずこれまでの経緯について。TiDBは外部キー制約が未サポートでしたが、v6.6より外部キー制約がサポートされました。
:::message
ただし、現在はまだExperimental Feature扱いとなっています。
:::

TiDBユーザー界隈からは「外部キー制約がサポートされたーー！！」という声を聞く一方で、いざ使ってみたらパフォーマンス観点で「およよ？」となった（ロック動作の違いにより）という話しも何度か耳にしたので、その点について特に見ていきます。
...
具体的には、TiDBで以下のような親テーブルのid列と子テーブルにおいて親を示すpid列とが外部キーにあるような関係性のテーブル構造において、親テーブルのレコードと親子関係にあるレコードを子テーブルに"大量に書き込み"（Insert,Update,Delete）があるようなワークロードでは、親レコードのロック待ちで思うようにパフォーマンスが出ないことがあります。
![](/images/34ae226f21e250/tableimage01.png)

極端なケースですが、親テーブルに１レコードのみ存在している場合、子テーブルへの書き込みは常にその１レコードを参照することになるため、大量の書き込みがある場合はロック待ちとなるため性能面の影響を受けます。

特定の親テーブルのレコードに関連するレコードを子テーブルに大量に書き込む、というのがポイントです。

つまりこういうこと。右のように異なる親に少量の子が同時に追加される（子のレコードが書き込まれる）のは問題ないですが、
左のように同じ親に多数の子が同時に追加されるのはTiDBはロック待ちになり性能影響を受ける、となります。
（社会的には左のほうが望ましいと思いますけどね　※個人的な意見です）。
![](/images/34ae226f21e250/workloadimage01.png)

伝わりますかね？（笑）念のため、具体的な挙動について確認していきます。
...
### 準備：MySQL/TiDB共通
MySQL、TiDB双方ともに分離レベルはデフォルトのRepeatable Readで確認します。
...
```
続いて、MySQL、TiDB双方の親テーブル`parant`にレコードを登録しておきます。
```sql
...
```
ここからわかることとして、各トランザクションでparentテーブルのレコードに対して行（`LOCK_TYPE = RECORD`)に対する共有ロック（`LOCK_MODE = S,REC_NOT_GAP`）を取っています。
したがって、子テーブルのINSERT時に親テーブルを参照可能となり、無事INSERTが出来ることになります。
ちなみに（当たり前ですが）、このタイミングで他のトランザクションから親のレコードを更新することは出来ません。
```sql
...
mysql> INSERT INTO child (pid, name) VALUES (1, "やまだ じろう");
```
2つ目のセッションは書き込みが完了せず、MySQLと挙動が異なっていることが分かります。
...
それを実現するうえでのロックの仕組みが異なることから、当該ケースのように特定の親テーブルに関連するレコードを子テーブルに大量に書き込もうとすると、性能面のビハインドが生じる、ということになります。
...
ただ、検討した結果、外部キー制約そのものは利用したい（DB移行のため利用せざるを得ないなど）場合、以下を検討することをおすすめします。
